...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
	...
	{
    mexErrMsgTxt("PARAMS.x0 must be of size [180 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.x0,180);
    ...
    if( mxGetM(par) != 6800 || mxGetN(par) != 1 ) 
	{
    mexErrMsgTxt("PARAMS.all_parameters must be of size [6800 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.all_parameters,6800);
	...
	copyCArrayToM_double( output.x01, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x01", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x02, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x02", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x03, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x03", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x04, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x04", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x05, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x05", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x06, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x06", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x07, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x07", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x08, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x08", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x09, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x09", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x10, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x10", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x11, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x11", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x12, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x12", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x13, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x13", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x14, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x14", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x15, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x15", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x16, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x16", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x17, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x17", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x18, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x18", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x19, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x19", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM_double( output.x20, mxGetPr(outvar), 9);
