...
void MPCC::jointStateCallBack(const sensor_msgs::JointState::ConstPtr& msg)
{
  ...
  for (unsigned int i = 0; i < degree_of_freedom_; ++i)
  {
    for (unsigned int j = 0; j < msg->name.size(); ++j)
    {
      ...
      if ( std::strcmp( msg->name[j].c_str(), controller_config_->joints_name_[i].c_str()) == 0 )
      {
  ...
  else
  {
    ...
    getTransform(controller_config_->chain_root_link_, target_frame_, goal_pose_);
...
void MPCC::publishErrorPose(const Eigen::VectorXd& error)
{
  ...
  stamped.header.frame_id = controller_config_->chain_root_link_;
...
void MPCC::publishTrajectory()
{
  ...
  //header
  marker.header.frame_id = controller_config_->chain_root_link_;
...
bool MPCC::checkInfinitesimalPose(const Eigen::VectorXd &pose)
{
  // check infinitesimal distance in position
  if (fabs(pose(0)) > controller_config_->goal_pose_tolerance_.at(0))
  {
    return false;
  }

  if (fabs(pose(1)) > controller_config_->goal_pose_tolerance_.at(1))
  {
    return false;
  }

  if (fabs(pose(2)) > controller_config_->goal_pose_tolerance_.at(2))
  {
    return false;
  }

  // check infinitesimal distance in orientation
  if (fabs(pose(3)) > controller_config_->goal_pose_tolerance_.at(3))
  {
    return false;
  }

  if (fabs(pose(4)) > controller_config_->goal_pose_tolerance_.at(4))
  {
    return false;
  }

  if (fabs(pose(5)) > controller_config_->goal_pose_tolerance_.at(5))
  {
    return false;
  }
...
bool MPCC::checkPositionLimitViolation(const Eigen::VectorXd &joint_position, const double &position_tolerance)
{
  ...
  for (int i = 0u; i < size; ++i )
  {
    ...
    if ( (min_position_limit_(i) - position_tolerance) >= joint_position(i) )
    {
      ROS_WARN("%s lower position tolerance violate with current position %f required position %f",
               controller_config_->joints_name_.at(i).c_str(),
...
void MPCC::enforcePositionInLimits(const Eigen::VectorXd &joint_position,
                                                 ...
                                                 const double& position_tolerance)
{
  ...
  for (int i = 0u; i < size; ++i )
  {
       ...
       )
    {
      ...
      ROS_INFO("%s new position %f", controller_config_->joints_name_.at(i).c_str(), min_position_limit_(i));
    }
            ...
            )
    {
      ...
      ROS_INFO("%s new position %f", controller_config_->joints_name_.at(i).c_str(), max_position_limit_(i));
    }
                                                 ...
                                                 const double &velocity_tolerance)
{
  ...
  for (int i = 0u; i < size; ++i )
  {
       ...
       )
    {
      ...
      ROS_INFO("%s new velocity %f", controller_config_->joints_name_.at(i).c_str(), min_velocity_limit_(i));
    }
            ...
            )
    {
      ...
      ROS_INFO("%s new velocity %f", controller_config_->joints_name_.at(i).c_str(), max_velocity_limit_(i));
    }
